...
def autogen_makefile(buildpara, MakePara, clist, assemble):
    dependencies = '-include $(wildcard $(BUILD_DIR)/*.d)\n'
    #check buildpara['BUILD_TO'] to get the generate path
    print("path:", buildpara['BUILD_TO'], "is the build dir")
    if os.path.isdir(buildpara['BUILD_TO']):print("the dir:",buildpara['BUILD_TO'], "is existed!")
    else:os.mkdir(buildpara['BUILD_TO'])
    ...
    if buildpara.__contains__('BUILD_TYPE') and buildpara['BUILD_TYPE'] == "release":fd.write("DEBUG = 0\n\n")
    else:fd.write("DEBUG = 1\n\n")
    ...
    for asfile in assemble:
        fd.write("\\\n" + asfile)
    fd.write("\n\n")
    if buildpara.__contains__('GCC_PATH'): fd.write(buildpara['GCC_PATH'] + "\n\n")
